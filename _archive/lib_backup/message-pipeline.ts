...
import { WhatsAppApiError, logWhatsAppEvent, sendWhatsAppMessage } from "@/lib/whatsapp";
...
async function queueOutboundMessage(input: {
  ...
  await recordSendAttempt({
    ...
    phone: input.phone,
    status: "QUEUED",
    reason: "Mensagem de conversa enfileirada para envio via worker.",
    queueRecordId: result.queueRecordId,
    metadata: {
      conversationId: input.conversationId,
      messageId: message.id,
...
export async function processOutgoingMessageJob(payload: OutgoingMessageJobPayload) {
  ...
  if (!gate.allowed) {
    await prisma.message.update({
      ...
      data: {
        ...
        complianceStatus:
          gate.status === "RATE_LIMITED" ? ComplianceStatus.PACED : ComplianceStatus.BLOCKED
      }
    });
  ...
  let sentMessage: Awaited<ReturnType<typeof sendWhatsAppMessage>>;

  try {
    sentMessage = await sendWhatsAppMessage(payload.phone, payload.text);
  } catch (error) {
    const messageText = error instanceof Error ? error.message : "Falha ao enviar mensagem pelo WhatsApp.";
  ...
  await recordSendAttempt({
    ...
    phone: payload.phone,
    status: "SENT",
    reason: "Mensagem de conversa aceita pela Meta.",
