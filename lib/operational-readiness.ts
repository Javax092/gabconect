...
export async function getOperationalReadiness(mandateId: string) {
  ...
  let queueCounts;
  let latestInboundMessage;
  let latestLog;
  let latestFailedLogs;
  let attemptGroups;
  let staleQueueCount;
  let completedQueueRecords;
  ...
  try {
    ...
    console.error("[operational-readiness] database readiness failed", error);

    return getDegradedOperationalReadiness(queueHealth, error);
  }
...
function getDegradedOperationalReadiness(queueHealth: Awaited<ReturnType<typeof getQueueHealth>>, error: unknown) {
  const databaseMessage = error instanceof Error ? error.message : "Falha ao consultar readiness operacional.";
  const envChecklist = [
    { key: "WHATSAPP_ACCESS_TOKEN", label: "Token de acesso Meta", configured: Boolean(process.env.WHATSAPP_ACCESS_TOKEN) },
    { key: "WHATSAPP_PHONE_NUMBER_ID", label: "Phone number ID", configured: Boolean(process.env.WHATSAPP_PHONE_NUMBER_ID) },
    { key: "WHATSAPP_VERIFY_TOKEN", label: "Verify token do webhook", configured: Boolean(process.env.WHATSAPP_VERIFY_TOKEN) },
  ...
  return {
    ...
    workers: ["incoming", "outgoing", "human"].map((name) => ({
      name,
      status: "unknown",
      note: "Banco indisponível para readiness.",
      lastSeenAt: null
    })),
    outgoingWorkerReady: false,
    operationalMetrics: {
      sent: 0,
      blocked: 0,
      optOuts: 0,
      ...
      deliveryRate: 0,
      pendingQueue: 0,
      averageSendTimeSeconds: 0,
      estimatedSafeCapacityPerHour: getEstimatedCampaignCapacityPerHour()
    },
    operationalAlerts: [
      {
        level: "CRITICAL" as const,
        type: "DATABASE_READINESS_UNAVAILABLE",
        message: `Banco indisponível para health/readiness: ${databaseMessage}`
      },
      ...(queueHealth.redis !== "ready"
        ? [
            {
    ...
    envChecklist,
    productionEnvChecklist: getProductionEnvChecklist().map((item) => ({
      ...item,
      status: getItemStatus(item.configured)
    })),
